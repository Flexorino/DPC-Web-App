...
export class AddEntryBSMeasureEntryComponent implements OnInit, AfterViewInit, Validator, ControlValueAccessor {
  ...
  private fastKE: ConstructionConstrol<ConstructionControlValue<FoodIntakeAttribute>> = new ConstructionConstrol(null);
  ...
  ngOnInit() {
    FormUtil.waitForInitialization(this.timeStampControl, this.bsMeasureControl, this.fastKE, this.correctionFoodBolusControl).subscribe(() => this.handleSubFormSubsciptions());
  ...
  private initializeForms() {
    ...
    this.mainFormGroup.setValidators([FormUtil.save(((x: AbstractControl) => this.bsMeasureControl.value.constructed || this.correctionFoodBolusControl.value.constructed || this.fastKE.value.constructed ? null : { atleastOneEntryAttributeNeedsToBeSet: null }))])
  }
  ...
  private handleSubFormSubsciptions() {
    FormUtil.getImmediateObservableFromRawControl(this.mainFormGroup).pipe(delay(0), map(() => {
      ...
      if (this.fastKE.value.constructed) {
        entry.foodIntakes.push(this.fastKE.value.constructed);
      }
